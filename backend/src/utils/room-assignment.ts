...
export const assignStudentsToRoom = async (
  ...
  try {
    ...
    // Find the room in the ClassType
    const room = classType.rooms.find((r) => r.roomNumber === roomNumber);
    if (!room) {
      throw new Error("Room not found in this ClassType");
    }
    ...
    // Add mandat numbers to the room in ClassType
    ...
    // Update the room's mandatNumbers array
    room.mandatNumbers.push(...mandatNumbers);
  ...
  try {
    ...
    for (const roomNumber of roomNumbers) {
      ...
      if (studentsForThisRoom.length > 0) {
        ...
        // Add mandat numbers to room
        const mandatNumbers = studentsForThisRoom
          .map((student) => student.mandatNumber)
          .filter((mandatNumber): mandatNumber is string => !!mandatNumber);
        room.mandatNumbers.push(...mandatNumbers);
    ...
    // Save the classType with updated rooms
    await classType.save();
