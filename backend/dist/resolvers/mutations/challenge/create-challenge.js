...
const ai_mappers_1 = require("@/services/ai.mappers");
const generated_1 = require("@/types/generated");
const createChallenge = async (_, { input }) => {
    try {
        ...
        if (input.taskCount < 1 || input.taskCount > 3) {
            throw new graphql_1.GraphQLError("Task count must be between 1 and 3 (limited due to OpenAI API constraints)");
        }
        ...
        if (input.taskDifficultyDistribution) {
            ...
            if (totalDistributedTasks !== input.taskCount) {
                throw new graphql_1.GraphQLError(`Task difficulty distribution mismatch: taskCount is ${input.taskCount} but difficulty distribution adds up to ${totalDistributedTasks}. Please ensure they match.`);
            }
        }
        // Validate ObjectIds
        if (!input.challenger || input.challenger.trim() === "") {
            throw new graphql_1.GraphQLError("Challenger ID is required and must be a valid ObjectId");
        }
        if (!input.opponent || input.opponent.trim() === "") {
            throw new graphql_1.GraphQLError("Opponent ID is required and must be a valid ObjectId");
        }
        // Validate topic is appropriate for the grade level
        if (!ai_mappers_1.AIMappers.validateTopicForGrade(input.topic, input.classType)) {
            const gradeNumber = ai_mappers_1.AIMappers.getGradeNumber(input.classType);
            let allowedTopics;
            if (gradeNumber <= 3) {
            ...
            throw new graphql_1.GraphQLError(`Topic ${input.topic} is not appropriate for Grade ${gradeNumber}. Allowed topics for this grade: ${allowedTopics}`);
        }
        ...
        if (input.answerFormatDistribution) {
            ...
            const mapChallengeFormatToAnswerFormat = (format) => {
                switch (format) {
                    case "MULTIPLE_CHOICE":
                        return generated_1.AnswerFormat.MultipleChoice;
                    case "SHORT_TEXT":
                        return generated_1.AnswerFormat.ShortText;
                    default:
                        return generated_1.AnswerFormat.MultipleChoice;
                }
            };
            ...
            generatedTasks =
                await taskGenerator_service_1.TaskGeneratorService.generateMultipleTasksWithFormatOptions({
                    topic: input.topic,
                    type: generated_1.TaskType.Challenge,
                    ...
                    difficultyDistribution: input.taskDifficultyDistribution || {
                        ...
                        easy: input.difficulty === "EASY"
                            ? taskCount
                            : Math.floor(taskCount * 0.3),
                        medium: input.difficulty === "MEDIUM"
                            ? taskCount
                            : Math.floor(taskCount * 0.5),
                        hard: input.difficulty === "HARD"
                            ? taskCount
            ...
            generatedTasks = await taskGenerator_service_1.TaskGeneratorService.generateMultipleTasks({
                topic: input.topic,
                type: generated_1.TaskType.Challenge,
                classType: input.classType,
                piPoints: piPointsPerTask,
                taskCount: taskCount,
                answerFormat: generated_1.AnswerFormat.MultipleChoice,
        ...
        const challenge = await models_1.ChallengeModel.create({
        ...
        throw new graphql_1.GraphQLError(`Failed to create challenge: ${errorMessage}`);
    }
};
